...
RsaCommunicator::RsaCommunicator(queue<const char*>& msgs) : _msgs(msgs)
{
...
RsaCommunicator::~RsaCommunicator()
{
	try
	{
...
void RsaCommunicator::bindAndListen()
{
	try {
...
void RsaCommunicator::handleNewClient(SOCKET client_socket)
{
	try {
		while (true) {
			char buf[1024];
